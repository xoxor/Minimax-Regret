...
\begin{proposition}\label{claim:PMR}
	...
	\begin{align} 
		\PMR(x,y; \pprofile, W)  
		& = \max_{w \in W} \sum_{j \in N} w_{\hat{v}_j(y)} - w_{\hat{v}_j(x)} \\ 
...
\commentBN{In the rebuttal we add: There is no a priori reason to believe that questioning the chair in terms of this different parametric space would yield logically equivalent answers: research in experimental psychology shows that participants’ answers differ widely when changing the phrasing of preference-related questions [Lichtenstein and Slovic,	2012, doi: 10.1017/CBO9780511618031]. To get out of such conundrums, we need a language considered “fundamental”. Questions of the form “In this profile, who should win?” arguably provides such a natural language.}
...
\begin{example*}
	...
	We show the following profile to the chair and ask who should win (each column is a preference of an agent).
	\begin{center}
		$
	...
	Note that we can compress this profile by grouping agents with the same preference orders. In the proof, in fact, we show how to construct a profile where only four linear orders are possible. Each column of the new profile represent now the preference of a group of agents:
	\begin{center}
		$
		\begin{array}{cccc}
...
To generate the scoring rule weights, we first draw $m \!-\! 1$ numbers uniformly at random (in the interval $\intvl{0,1}$ representing weight ``differences''), normalize and sort them; a sequence of convex decreasing weightx is then obtained by a decumulative sum. The penalty parameters for the Pessimistic and Extended pessimistic strategies are  $\epsilon_{\text{chair}} = 1.1$, $\epsilon'_{\text{chair}} = 10^{-6}$, $\epsilon_{\text{agent}} = 1.0$ and $\epsilon'_{\text{agent}} = 0$.
...
\begin{table}[ht]
	\caption{Analysis of the questions asked by Pessimistic strategy on the datasets of \cref{fig:linearity}. The column $q_{c TR}$ ($q_{c ZR}$) indicates the average number of questions asked to the chair to reach a threshold regret of $\frac{n}{10}$ (zero regret). The column $[Q1_{v} | M_{v} | Q3_{v}]_{TR}$ ($[Q1_{v} | M_{v} | Q3_{v}]_{ZR}$) represents the average of the quartiles of the questions asked to the voters to reach $\frac{n}{10}$ regret (zero regret).}
	\label{tab:questions}
	\begin{tabular}{cccS[table-number-alignment = right, table-figures-integer=2]@{ ± }S[table-number-alignment = left, table-figures-integer=2]cS[table-number-alignment = right, table-figures-integer=2]@{ ± }S[table-number-alignment = left, table-figures-integer=2]c}
		\toprule
		{dataset} & m & n &{$q_{c TR}$} & {sd} & {$[Q1_{v} | M_{v} | Q3_{v}]_{TR}$} & {$q_{c ZR}$} & {sd} & {$[Q1_{v} | M_{v} | Q3_{v}]_{ZR}$} \\
		\midrule
		m5n20 & 5&20&0&0&[4.3 | 4.95 | 5.84 ]&5.25&9.59&[5.36 | 6.15 | 7.21]\\
		m10n20&10&20&0&0&[13.85 | 16.1| 18.41]&31.95&26.62&[19.66 | 21.78 | 24.7]\\
		m11n30&11&30&0&0&[16.55|19|22.26]&45.15&40.14&[23.07|25.7|28.89]\\
		tshirts&11&30&0&0&[12.92 | 16.17 |19.3]&\textendash&\textendash&\textendash\\
		courses&9&146&0&0&[6.35 |7 |7]&0&0&[6.8 | 7 |7]\\
		...
		skate&14&9&0&0&[11.35|11.8|12.25]&0&0&[11.5|11.9|12.7] \\
		m15n30&15&30&0&0&[24.95|29.5|33.68]&\textendash&\textendash&\textendash \\
...
\begin{table}
	...
	\begin{tabular}{S[table-figures-integer=3, table-figures-decimal=0]S[table-number-alignment = right]@{ ± }S[table-number-alignment = left, table-figures-integer=1]S[table-number-alignment = right]@{ ± }S[table-number-alignment = left, table-figures-integer=1]}
		...
		\midrule
		0 & 0.86 & 0.72 & 0.73 & 0.67 \\
		15 & 0.92 & 0.82 & 0.93	& 0.96  \\
		30 & 0.9 & 0.97 & 0.72	& 0.7 \\
		50 & 1.36 & 1.29 & 1.38	& 1.17 \\
		100 & 1.9 & 1.72 & 2.5	& 1.46  \\
		%150 & 4.4 & 2.09 & 4.23 & 1.53 \\
		200 & 6.05	& 1.53  & 6.06 & 1.61 \\
		%250 & 8.27	& 1.54& 8.86 & 1.38 \\
		300 &11.54	& 1.07& 11.82 &1.25 \\
		%350 &15.39	& 0.61 &15.54	&0.73 \\
