...
\newcommand{\commentPV}[1]{\textcolor{brown}{\small$\big[$PV: #1$\big]$}}
...
\commentOC{Remaining refs to appendix. Describe what is there. data set VS dataset. Figure 2. Footnotes. Comments. Talk about small settings?}
\commentPV{If we have space (can save by formatting the itemize at the end of experiments as text), show Regret and PMR formulas as displaymath}
...
Our approach is evaluated on simulations with synthetic and real datasets where both the voting rule and the agent preferences are initially unknown to the system and incrementally revealed through questioning.
...
The following propositions (proofs in appendix) show that the procedure for completing a partial profile,  proposed by \citet{Lu2011} when considering a fixed weight vector, also applies in our setting. We write $a \pprefeq_j b$ iff $a \ppref_j b \lor a = b$ and adopt the canonical notation when considering a relation as a function, writing ${\pprefeq_j}(x)$ to designate $\set{y \suchthat x \pprefeq_j y}$.
...
This is similar to favor, in decision theory, direct choice questions ("please choose either a or b") compared to questioning the decision maker about the shape of her utility function. The former are considered “observable”: acts of choice are translated to preference statements \citep[Ch.\ 1]{colell_microeconomic_1995}. 
...
%Responses to questions involving $x^*$ or $\bar{y}$ constrain the ability of the adversary to complete the profile in such manner, possibly reducing minimax regret.
	...
	In other words if the maximal {\em a posteriori} MMR of two questions are (nearly) equal, then it considers the (penalized) minimal MMR values, preferring the question with the lowest value.\footnote{In our tests 
	%the parameters for Pessimistic and Extended pessimistic are 
	we set to $\epsilon_{\text{chair}} = 1.1$, $\epsilon'_{\text{chair}} = 10^{-6}$, $\epsilon_{\text{agent}} = 1.0$ and $\epsilon'_{\text{agent}} = 0$.}
...
\commentPV{Check problem size displayed in the labels of the two table. The one in the appendix does not seem larger.}
...
%			\addlegendentry{m=5, n=15}
			...
			\addplot[thick, mark=halfsquare left*, mark size = {2}, mark indices = {200}, blue] table [x=k, y=14.9]{data/m14n9.dat};
			...
			\addplot[thick, mark=halfsquare left*, mark size = {2}, mark indices = {200}, orange] table [x=k, y=14.10]{data/new.dat};
			\addlegendentry{m=14, n=10}
			\addplot[thick, mark=halfsquare right*, mark size = {2}, mark indices = {60}, black] table [x=k, y=15.10]{data/new.dat};
			\addlegendentry{m=15, n=10}
			\addplot[thick, mark=halfsquare*, mark size = {2}, mark indices = {400}, yellow] table [x=k, y=15.9]{data/new.dat};
			\addlegendentry{m=15, n=9}
		\end{axis}
	\end{tikzpicture}
\end{figure}

%\begin{table}
%	\caption{Average number of questions asked to the chair (\textit{qstC}) and to the agents (\textit{qstV}) by Pessimistic strategy to reach zero regret on the data sets of \cref{fig:linearity}.}
%	\label{tab:questions}
%	\begin{tabular}{S[table-text-alignment=right] S[table-number-alignment = right]@{ ± }S[table-number-alignment = left, table-figures-integer=1]S[table-number-alignment = right]@{ ± }S[table-number-alignment = left, table-figures-integer=1]}
%		\toprule
%		{data set} & {qstC} & {sd} & {qstV} & {sd} \\
%		\midrule
...
\commentPV{If are evenly distributed, then doesn't this goes against our argument?}
...
We summarize the main observations from the experiments:
\commentPV{We could save space by 'de-itemizing' this, i.e. by formatting as text with i) bla bla bla.... ii) .... iiii) ....}
\begin{itemize}
