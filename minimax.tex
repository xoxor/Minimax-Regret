...
\begin{example*}
	...
	Figure \ref{fig:profileQstCommCompact} shows the same profile using a compressed view, the numbers in bold indicating the number of agents having the preference in the corresponding column. As the proof shows, constructed profiles require only four different linear orders.
	\begin{figure}
		\centering
		...
		\end{subfigure}
		\caption{Profile representing a question for the chair in extended (a) and compact (b) form.}
	\end{figure}
\end{example*}
...
\begin{table}[ht]
	\caption{Analysis of the questions asked by Pessimistic strategy on the datasets of \cref{fig:linearity}. The column $q_{c \scriptscriptstyle{\MMR} ≤ n/10}$ ($q_{c \scriptscriptstyle{\MMR} = 0}$) indicates the average number of questions asked to the chair to reach a threshold regret of $\frac{n}{10}$ (zero regret). The column $q_{v \scriptscriptstyle{\MMR} ≤ n/10}$ ($q_{v \scriptscriptstyle{\MMR} = 0}$) represents the average of the quartiles of the questions asked to the voters to reach $\frac{n}{10}$ regret (zero regret).}
	\label{tab:questions}
	\begin{tabular}{cccS[table-number-alignment = right, table-figures-integer=2]@{ ± }S[table-number-alignment = left, table-figures-integer=2] S[table-figures-integer=3, table-figures-decimal=1]@{ | }S[table-figures-integer=2, table-figures-decimal=1]@{ | }S[table-figures-integer=2, table-figures-decimal=1]@{ ]} S[table-number-alignment = right, table-figures-integer=2]@{ ± }S[table-number-alignment = left, table-figures-integer=2]S[table-figures-integer=3, table-figures-decimal=1]@{ | }S[table-figures-integer=2, table-figures-decimal=1]@{ | }S[table-figures-integer=2, table-figures-decimal=1]@{ ]}}
		...
		m15n30&15&30&0.0&0.0&[24.95&29.5&33.68&\multicolumn{2}{c}{---}&\multicolumn{3}{c}{---} \\
...
\begin{table}
	...
	\begin{tabular}{S[table-figures-integer=3, table-figures-decimal=0]S[table-number-alignment = right]@{ ± }S[table-number-alignment = left, table-figures-integer=1]S[table-number-alignment = right]@{ ± }S[table-number-alignment = left, table-figures-integer=1]}
		...
		{$q_c$} & {2 ph.\ ca} & {sd} & {2 ph.\ ac} & {sd} \\
		\midrule
		0 & 0.78 & 0.38 & 0.73 & 0.67 \\
		15 & 0.58 & 0.3 & 0.59	& 0.37  \\
		30 & 0.29 & 0.31 & 0.55 & 0.19 \\
		50 & 0.0 & 0.0 & 0.0 & 0.01 \\
		100 &  &  & 	&   \\
		200 & 	&   &  &  \\
		300 &	& &  & \\
