...
Third, we experimentally find out that asking questions to the voters only performs similarly than asking questions both to the voters and the chair, although questions to the chair do have a positive value in terms of quality of the recommendation.
...
In this section we want to contrast our approach to one that would obtain the full profile on the one hand, and determine the voting rule on the other hand. More precisely, we want to estimate the number of questions that would be required to elicit the complete profile. We focus on the profile because eliciting the scores to full precision would require infinitely many questions, with the questions we use. 
