...
\paragraph{Comparison of all strategies in problem instances of small size}
...
\begin{table}
	...
	\end{tabular}
\end{table}
%Our first experimental result concerns small size situations. 
...
We see that that Pessimistic performs better than Random,  Extended pessimistic, %. % on a small size problem.
and as well Elitist.
%(The figure also displays the performance of the Elitist strategy).
...
\begin{table}
	...
	\end{tabular}
\end{table}
While in previous experiments we compared Pessimistic to the other strategies, in this set of experiments, we focus %about the absolute quality of 
on Pessimistic. 
We %For practical usability,
...
The experiments so far let the strategy target its questions unrestrictively, leaving open the possibility of questioning either the chair or an agent at each step. One may wonder what is lost in terms of regret by asking different proportions of questions to the chair and the agents. Such restrictions may be useful because of (partial) unavailability of the chair, or because the estimated cognitive costs may differ sensibly, as these types of questions differ in nature. \Cref{tab:twoP400} shows the $\MMR$ value reached in a problem of size $m = 10, n = 20$ after $400$ questions, using the Two phases strategy, in the “ca” ({\em chair then agents}) and in the “ac” ({\em agents then chair}) variants. 
The former variant first asks $q_{c}$ questions to the chair and then $400-q_{c}$ to the agents; the latter first $400-q_{c}$ to the agents and then $q_{c}$ questions to the chair.
 These numbers are to be compared with the $\MMR$ value reached after 400 questions with the unconstrained Pessimistic strategy (displayed in \cref{fig:linearity}), which is $0.62$.% ± $0.56$.
