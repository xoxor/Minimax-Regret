...
\Cref{sec:background} provides the necessary background and \cref{sec:mmr} introduces the minimax criterion, a method that selects a winner that minimizes the worst possible regret.
In Section \ref{sec:elicit} we provide an interactive elicitation protocol based on minimal regret;  Section \ref{sec:experiments} presents the experiments validation of our approach and the guidelines stemming from these simulations; and Section \ref{sec:conclusions} provides some final thoughts.
...
\Citet{Cailloux2014} are interested in elicitation of a different class of rules.
...
%$150$ questions?
%Pessimistic systematically better than XPess?
%Page numbers
%reflexive?
%\end{enumerate}
...
We also see that Pessimistic performs slightly better than Extended pessimistic, showing that Pessimistic chooses candidate questions wisely; this is good news since Pessimistic is much faster: it takes on average only $24$s for a complete elicitation session (for $m = 5$, $n = 10$ and $150$ questions), while Extended pessimistic takes $76$s. Notice that although they exhibit a close performance, Pessimistic performs %significantly %better, in the sense that the numbers are 
