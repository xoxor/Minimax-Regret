...
We compare the effectiveness of several strategies  including strategies that proceed in phases, learning first as much as possible about the voting rule and then focus on eliciting agent's preferences, and as well strategies that interwind \commentBN{Is it a real word? The dictionary corrects me with https://dictionary.cambridge.org/it/dizionario/inglese/intertwined} questions to chair and to the agents based on which is estimated to provide more information.
In this work:
\begin{itemize}
	\item we provide a new mechanism for the elicitation of the voting rule which translate abstract questions about weights to direct choice questions about a profile;  
	\item we show that, starting with zero knowledge, it is possible to reach a reasonable low regret level with a reasonable number of questions;
	\item we present different elicitation strategies proving that it is possible to achieve good results without sacrificing computation time;
	\item we show that for the class of rule considered, only few questions about the voting rule are enough to reach a low regret level;
	\item we show that low degree of similarity among preference rankings (like in the case of impartial culture) is a more challenging setting than less varied profiles.
\end{itemize}
