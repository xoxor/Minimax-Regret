...
Second, the decrease in $\MMR$ is approximately linear with the number of questions asked, except when the regret approaches zero. \commentBN{Not true anymore, some example with Pess 
$\begin{array}{ccc}
...
The \strat{Extended pessimistic} strategy uses the same criterion as the pessimistic strategy, but extending it to a bigger set of $m² n + m$ candidate questions, the same as those considered by the Random strategy.
...
The \strat{Two phases - ac} variant first asks $p$ questions to the agents, then $k-p$ questions to the chair, using in both cases Pessimistic to select the specific questions; whereas the \strat{Two phases - ca} variant starts with $p$ questions to the chair, then questions the agents. 
...
\commentOC{
	\begin{enumerate}
		\item Pessimistic performs best.
		\begin{itemize}
			\item Much better than Random
			\item Better than XPess and Elitist!
			\item Graph featuring (5, 10, 150, 200 runs), Random, XPess, Pess, Elitist.
			\item And table like table 1.
		\end{itemize}
		\item Pess performs well.
		\begin{itemize}
			...
			\item Much faster than XPess [do this much later]
		\end{itemize}
		...
		\end{itemize}
		\item Pessimistic approx Optimistic [do this much later]
	\end{enumerate}
	TODO: find a way of displaying the graph so that it is understandable in B/W.
	Variations around the small size. Time comparison XPess VS Pess?
	Check Pess VS XPess on other sizes.
}
...
\subsection{Judging pessimistic relatively}
\begin{table}
	\begin{center}
...
%We observe that the Pessimistic strategy is able to reduce the regret almost completely after $30$ questions. We also see that Pessimistic performs better than Extended pessimistic, showing that Pessimistic chooses candidate questions wisely; this is good news since (for $m=5$ and $n=5$), the latter strategy is much faster and takes only $2.6s$ for a complete elicitation session, while the former takes $16s$.
...
\subsection{Judging Pessimistic absolutely}
\label{sec:lowRegret}
\begin{table}
	\begin{center}
