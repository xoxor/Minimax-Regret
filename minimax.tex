...
\commentOC{
	\begin{enumerate}
		\item Limited pessimistic performs best.
		\begin{itemize}
			\item Much better than Random
			\item Better than Pess and Elitist!
			\item Graph featuring (5, 10, 150, 200 runs), Random, Pess, L. pess, Elitist.
			\item And table like table 1.
		\end{itemize}
		\item L pess performs well.
		\begin{itemize}
			...
			\item Much faster than Pess [do this much later]
		\end{itemize}
	...
	\end{enumerate}
	TODO: find a way of displaying the graph so that it is understandable in B/W.
	Variations around the small size. Time comparison Pess VS LPess?
}
...
\subsection{Judging limited pessimistic relatively}
\begin{table}
	\begin{center}
...
We see that that Limited pessimistic performs better than Random and Pessimistic on a small size problem.
(These figures also display the performance of the Elitist strategy, to which we will come back.)

We observe that the Pessimistic strategy is able to reduce the regret almost completely after $30$ questions. We also see that Limited pessimistic performs better than Pessimistic, showing that Limited pessimistic chooses candidate questions wisely; this is good news since (for $m=5$ and $n=5$), the latter strategy is much faster and takes only $2.6s$ for a complete elicitation session, while the former takes $16s$.

Asking Random questions does not permit to reach a low regret level even after having asked 100 questions, whereas a low regret level (MMR = 1) is reached by Limited pessimistic before having asked 60 questions.

We also see that Pessimistic performs slightly less well than Limited pessimistic. Although they come close, Pessimistic performs significatively less well, in the sense that the numbers are consistently in favor of Limited pessimistic if running the experiment again.

We next focus on the Limited pessimistic strategy, because Pessimistic is too slow to be ran repeatedly on much bigger problem sizes.

\subsection{Judging limited pessimistic absolutely}
\label{sec:lowRegret}
\begin{table}
	\begin{center}
...
\subsection{Comparing with full elicitation}
In this work, we advocate obtaining an approximate winner without obtaining the full profile or determine completely the voting rule. The previous section evaluate the number of questions per voter to reach a low regret value.
In order to contrast our approach to one that would apply existing works to, on the one hand, obtain the full profile, and on the other hand, to determine the voting rule, it is interesting to compute the number of questions that would be required to elicit the complete profile. We focus on the profile because eliciting the scores to full precision would require infinitely many questions, with the questions we use, and existing works do not propose to use such questions, thus they are not readily comparable on that aspect. 
...
To estimate this number, we assume the following querying process: given a partial ordering representing our knowledge so far of an agent preferences, we pick a question equiprobably among the remaining edges, and add the edge in the appropriate direction to our partial ordering. We count the average number of questions required to reach a complete ordering, assuming that every linear order is equally probable as a representation of the real agent preferences.
...
The experiments so far let the strategy decide the target of its questions, leaving open the possibility of questioning either the chair or an agent at each step. It may be desired in practical implementations to restrict the strategy to ask questions to agents only, or mostly, or to wonder what is lost in terms of regret by asking more questions to the chair, or what is lost if the circumstances require to ask all questions to the chair first, then only question the voters, or the other way around. Such restrictions may be useful because of (partial) unavailability of the chair, or because the estimated cognitive costs may differ sensibly, as these types of questions differ in nature. \Cref{tab:twoP400} show the MMR value that is reached in a problem of size $m = 10, n = 20$ after $400$ questions, using various constraints added to the Limited pessimistic strategy. The constraints have this form: the strategy must ask $p$ questions to the chair first, then $400 - p$ questions to the agents; or conversely, must ask $400 - p$ questions to the agents before turning to the chair. These numbers are to be compared with the MMR value reached after 400 questions with the unconstrained Limited pessimistic strategy (displayed in \cref{fig:linearity}), which is $0.62$.% ± $0.56$.
