...
\commentOC{Remaining refs to appendix. Describe what is there.}
...
\commentBN{Either we remove this consideration about n/10 together with \cref{tab:lowRegret}, and we present \cref{tab:questions}. Or we redo \cref{tab:questions} considering the questions needed to reach MMR=n/10 instead of the ones needed to reach 0.}
...
\Cref{fig:linearity} shows graphically the decrease in MMR according to the number of questions asked for various problem sizes. In particular, we can observe, as we expected, that given a problem size, the less the profile is varied the more questions are needed to reach a given MMR. For instance, in the \textit{Skate} dataset the top-2 alternatives are the same for all agents, and 8 out of 9 agents rank the same alternative at position 3; in \textit{Courses} all agents rank the same alternative at the top position, while in \textit{T Shirt} the alternatives are evenly distributed in the preference rankings. If we compare \textit{Skate} with the same size generated problem (m=14, n=9) we see that the former performs much better than the latter, reaching zero regret after only $100$ questions, while the IC needs almost $500$ questions to reach the same result. But if we compare \textit{T Shirt} with the corresponding IC, \commentBN{Do we substitute m=10, n=30 with m=11, n=30 to make it same as T shirt?} we do not observe any important differences in terms of decrease of regret. This suggest that high degree of similarity in the preference rankings helps reducing the regret faster.
