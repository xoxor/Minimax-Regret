...
Notice \commentOC{?} that if $\MMR(\pprofile, W)\!=\!0$, then $x^{*}_{\pprofile,W}$ is a necessary co-winner; this means that for any valid completion of the profile and any feasible $w \!\in\! W$, $x^{*}_{\pprofile,W}$ obtains a highest score.
...
%A scoring rule associates each alternative to a score that is given by the sum of points obtained for each agent.
...
\commentOC{Recall that, in the computation of $s(x; \profile, \w)$, $w_{v_j(x)}$ represents the score that $x$ obtains in the ranking $v_j$ (see Equation \ref{eq:srule}).
Since scoring rules are additively decomposable, we can decompose the score in terms of votes. Thus, we can write the actual regret of choosing $x$ instead of $y$ as}
\[
...
Consider our knowledge $\ppref_j$ about the preference of the agent $j$. 
The adversary's goal is to make the score of $y$ as high as possible and the score of $x$ as low as possible. 
To do this, he should complete $\ppref_j$ to $\pref_j$ by putting above $x$ as many alternatives as he can, that is, all the alternatives except those that are known to be worse than $x$ (those $a$ such that $x \pprefeq_j a$); and similarly, put below $y$ all the alternatives he can, thus, put above $y$ only those $a$ such that $a \ppref_j y$. 
Sometimes these two objectives conflict: when an alternative should go above $x$ according to the first objective and below $y$ according to the second one (because $¬(x \pprefeq_j a) ∧ ¬(a \ppref_j y)$), and $x$ is known to be better than $y$ (thus $x \pprefeq_j y$). 
...
y \prefeq_j a &⇔ ¬(a \ppref_j y) ∧ (¬(x \pprefeq_j y) ∨ (x \pprefeq_j a)).
\end{split}
\end{align} 
...
is $\hat{v}_{j}(x) = 1+\card{A}-\card{{\pprefeq_j}(x)}$, where ${\pprefeq_j}(x)$ designates the set of alternatives that are less good than $x$ according to $\pprefeq_j$ (thus including $x$ itself) \commentOC{Is it not better write $\hat{v}_{j}(x) = 1+\card{A}-\card{{\ppref_j}(x)}$? I guess it's for coherence for the $\beta$ expression but it doesn't make much sense pointing out we're removing and adding x.}, and the rank of $y$ is $\hat{v}_{j}(y)=1+\card{{\pprefinv_j}(y)}+\beta$, where $\beta = \card{{\pinc_j}(x)}$ if $(x \pprefeq_j y)$ and $\beta = 0$ otherwise and with ${\pinc_j}(x)$ designating the alternatives incomparable with $x$ in $\pprefeq_j$ \commentOC{Still not good. In the set of alternatives incomparable with $x$ there could be alternatives strictly better than $y$. It is exactly the problem we had before. Instead, $\beta$ represents the number of alternatives incomparable with $x$ that are no better than $y$: $\beta = \card{A \setminus ({\pprefeq_j}(x) \cup {\pprefinv_j}(y))}$ }.
...
\end{align}
where the $\hat{\alpha}_{r}^{y}$ (resp. $\hat{\alpha}_{r}^{x}$)  is the number of times $y$ (resp. $x$) is at rank $r$ in the rankings of the completed \commentOC{?} profile $\hat{\profile}$.
\end{claim}
...
Notice \commentOC{?} that, whenever the PMR of an alternative $x$ (against some other alternative) exceeds the best MR value found so far, we don't need to further evaluate $x$, as it can't be optimal. \commentOC{?} 
