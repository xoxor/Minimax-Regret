...
As a first attempt to make this question concrete, we could define a profile $P$ such that there is an alternative $a$ that receives $q$ times the rank $r$ and $p$ times the rank $r+2$, and an alternative $b$ that receives $p+q$ times the rank $r+1$. 
...
However, we still need to complete the profile, in a way that none of these other alternatives is better than $a$ or $b$: if the chair picks a different alternative $c$, it tells us that the chair prefers $c$ to both $a$ and $b$, 
...
We can prove that, for $\delta = p+q$, and whatever the weight vector $\w \in \W$, no alternative, except possibly $b$, has a better score than $a$. (In fact, as the construction will exhibit, they are all worst than both $a$ and $b$, but the weaker fact is enough for our claim.)
...
The \strat{Limited pessimistic} strategy uses the same criterion as the pessimistic strategy, but limiting it only to a small set of candidate questions. It considers the current solution of the $\MMR$, i.e. the minimax optimal alternative $x^{*}$ and the adversarial choices $\bar{y}, \bar{w}, \bar{v}$.
Then, it selects n questions (one per agent) by adopting the heuristic proposed by \citet{Lu2011}. If we consider an agent $j$, there are only three possible relations between the alternatives $x^{*}$ and $\bar{y}$ in her partial preference: either $x^{*} \ppref_j \bar{y}$, or $\bar{y} \ppref_j x^{*}$, or they are incomparable. As shown in the proof of Claim \ref{claim:completion}, in order to increase $\PMR(x^{*},\bar{y})$, an adversary should place as many alternatives as he can above $x^{*}$ in the first case, and between $\bar{y}$ and $x^{*}$ in the second. An intuitive way to reduce the pairwise max regret is, therefore, to directly ask questions involving such alternatives, hoping for an answer that would prevent the adversary to play his best game. If $x^{*}$ and $\bar{y}$ are incomparable, then we just ask the voter $j$ to compare them.
To conclude the set of candidate questions, the \strat{Limited pessimistic} strategy also selects a question to the committee. We define
\begin{align}
w^\tau & = \argmin_{\w \in W} s(\bar{y}; \bar{\profile}, \w) - s(x^{*}; \bar{\profile}, \w), \label{eq:tau} 
\end{align}
as the weight vector that minimize the $\PMR$ in the worst case.
...
We can see in Table \ref{fig:55} that while the \strat{Pessimistic} strategy is able to reduce the regret almost completely after $30$ questions, it takes $1000$ times more time than the \strat{Random} strategy ($16s$ of the Pessimistic compared to $0.014s$ of the Random). Moreover, the results of the \strat{Limited pessimistic}, that takes almost $80\%$ less of the time of the \strat{Pessimistic} ($2.6s$), are essentially the same.
