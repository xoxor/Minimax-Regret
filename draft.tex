...
We define here the various strategies we tested experimentally; a strategy %is a function (or a random process) that 
gives us, given the current partial knowledge $(\pprofile, W)$, a question that should be asked. % according to that strategy. 
...
%The \strat{Pessimistic} strategy considers as possible questions the ones defined by the \strat{Random} strategy (with the difference that a threshold is used: if for some rank $r$ the interval of possible values for $\lambda$ is not larger than $0.1$, the question is not considered). 
However, to avoid the absorption property of max, we adopt the $\leximax$ criterion %a variant of the $\max$ 
as an aggregator: if the maximal $\MMR$ of two questions are equal, then it considers the other $\MMR$ values, (the ones associated to the opposite answer), preferring the question with the lowest of this value.
...
For each voter, a candidate question is chosen by considering the heuristic proposed by Lu and Boutilier \cite{Lu2011}. 
For the chair, we define
\begin{align}
\tau & = \min_{\w \in W} s(\bar{y}; \bar{\profile}, \w) - s(x^{*}; \bar{\profile}, \w),  \label{eq:tau1} 
\end{align}
and compare component-wise $\bar{\w}$ and $\w^{\tau}$, the argmax of Eq. (\ref{eq:tau1}), to find the position $r$ that is most valuable to ask about (the one that maximizes $\abs{\bar{\w}_r - \w^{\tau}_r}$), and choose $\lambda$ as the middle of the interval of possible values for that rank.
