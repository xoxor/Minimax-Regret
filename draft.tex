...
y \pref_j a &⇔ ¬(a \ppref_j y) ∧ (¬(x \pprefeq_j y) ∨ (x \pprefeq_j a)).
\end{split}
\end{align} 
\commentBN{I think it would be more intuitive writing: \\ $ y \pref_j a ⇔ ¬(a \ppref_j y) ∧ ¬((x \pprefeq_j y) ∧ ¬(x \pprefeq_j a))$}
...
is $\hat{v}_{j}(x) = 1+\card{A}-\card{{\pprefeq_j}(x)}$, where ${\pprefeq_j}(x)$ designates the set of alternatives that are less good than $x$ according to $\pprefeq_j$ (thus including $x$ itself), and the rank of $y$ is $\hat{v}_{j}(y)=1+\card{{\pprefinv_j}(y)}+\card{\beta}$, where $\card{\beta} = \card{A \setminus ({\pprefeq_j}(x) \cup {\pprefinv_j}(y))}$ if $(x \pprefeq_j y)$ and $\card{\beta} = 0$ otherwise.
In the case $(x \pprefeq_j y)$, intuitively $\beta$ represents the number of alternatives incomparable with $x$ that are no better than $y$. Let's reason about what alternatives must be placed above $y$ by negating the second equation of \ref{eq:compl}:
\begin{align}
\label{eq:betaproof} 
\begin{split}
a \prefeq_j y &⇔ (a \ppref_j y) ∨ ((x \pprefeq_j y) ∧ ¬(x \pprefeq_j a))\\
a \pref_j y &⇔ (a \neq_j y) ∧ [(a \ppref_j y) ∨ ((x \pprefeq_j y) ∧ ¬(x \pprefeq_j a))]
\end{split}
\end{align} 
But the term in the square brackets implies $(a \neq_j y)$, indeed:
\[(a \ppref_j y) ∨ ((x \pprefeq_j y) ∧ ¬(x \pprefeq_j a)) \Rightarrow (a \neq_j y)\]
$(a \ppref_j y) \Rightarrow (a \neq_j y)$ is always true, and we can rewrite $¬(x \pprefeq_j a)$ as $(a \ppref_j x) ∨ (a \pinc_j x)$, where $\pinc_j$ designates incomparability. Thus:
\begin{gather*} 
(x \pprefeq_j y) ∧ ¬(x \pprefeq_j a) \Rightarrow (a \neq_j y) \\
(x \pprefeq_j y) ∧ ((a \ppref_j x) ∨ (a \pinc_j x)) \Rightarrow (a \neq_j y) \\
((x \pprefeq_j y) ∧ (a \ppref_j x)) ∨ ((x \pprefeq_j y) ∧ (a \pinc_j x)) \Rightarrow (a \neq_j y).
\end{gather*}
So we can go back to \ref{eq:betaproof} dropping the $(a \neq_j y)$ term:
\[a \pref_j y ⇔ (a \ppref_j y) ∨ ((x \pprefeq_j y) ∧ ¬(x \pprefeq_j a) ∧ ¬(a \ppref_j y))\]
In case $(x \pprefeq_j y)$, we say an alternative $a \in \beta$ iff $¬((x \pprefeq_j a) ∨ (a \ppref_j y))$. Thus defining $\beta=A \setminus ({\pprefeq_j}(x) \cup {\pprefinv_j}(y))$.
