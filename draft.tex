...
{\color{red}
\begin{align}
	\tau_{\w} & = \min_{\w \in W} s^{\bar{\profile},\w}(\bar{y}) - s^{\bar{\profile},\w}(x^{*}) \\
	\tau_{\profile_i} & = \min_{\hat{\profile_i} \in C(\pprofile)} s^{\hat{\profile_i},\bar{\w}}(\bar{y}) -  s^{\hat{\profile_i},\bar{\w}}(x^{*}) 
\end{align}
where $\hat{\profile_i}$ is defined as $\bar{\profile}$ except for $i$, $\forall i \in \{1, \dots, n\}$. }
