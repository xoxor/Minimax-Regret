...
\newcommand{\w}{\bm{w}}%partial weights
...
%With a little abuse of terminology, we will use the term profile to refer to either $\profile$ or to the preference relations, depending on the context.
...
An alternative obtains a score that depends on the rank obtained in each of the preference orders \commentOC{Consider removing $v$ and $w$}:
\begin{align}
\label{eq:srule}
...
We use $\C(\w^\text{p})$ to denote the set of weight vectors compatible with the preferences expressed by the chair about the scoring vector. \commentOC{Think about this.}
