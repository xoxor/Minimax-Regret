...
We assume defined a set $A$ of $m$ alternatives (products, restaurants, movies, public projects, job candidates, etc.) and a set $\set{1, …, n}$ of agents (voters); each agent $j$ is associated to her “real” preference order $\pref_j$. The agents come from an infinite set $\N$ of potential voters.
...
Following the social choice nomenclature, we call {\em profile} the association of a preference to each voter, considering a subset of voters from the set $\N$, and denote a profile by $(\pref_1,\ldots,\pref_n)$.
...
We use $\pw \subseteq \W$ to denote the set of weight vectors compatible with the preferences expressed by the chair about the scoring vector, and $C_W$ to denote the set of linear constraints expressed by the chair.
...
Notice that if $MMR(\pprofile, W)=0$, then $x^{*}_{\pprofile,W}$  is a necessary co-winner; this means that for any valid completition of the profile and any feasible weight $w \in W$, $x^{*}_{\pprofile,W}$ obtains a highest score.
...
To summarize:\commentOC{TODO only one number here} 
\begin{align} 
\label{eq:compl}
...
The space of feasible weights $W$ is encoded with linear constraints that model the preferences of the chair.
...
First of all, we discuss the different types of questions that can be asked to the voters and to the chair, and the way responses are handled.
...
The effect of a response to a question asked to a voter is to augment our knowledge about the voters rankings, thus augmenting the partial profile $\pprofile$. 
...
Observe that the question may be defined equivalently as whether $q w_{r} + p w_{r+2} ≥ (p + q) w_{r+1}$, where $p, q$ are natural numbers. As a first attempt to make this question concrete, let us define a profile $P$ containing $p+q$ voters. Define an alternative $a$ that receives $q$ times the rank $r$ and $p$ times the rank $r+2$ and an alternative $b$ that receives $p+q$ times the rank $r+1$. Observing that the score of $a$ in that profile is exactly the left hand side of the question, and that the score of $b$ is the right hand side, intuition suggests that observing whether the chair picks $a$ or $b$ as winner will let us determine which side is greater; equality occurring when the chair declares $a$ and $b$ as tied for the victory. However, we still need to complete the profile, thus, come up with other $m-2$ alternatives defined so that each voter in the resulting profile has given each rank once. And we must ensure, doing this, that these other alternatives are not better than $a$ or $b$. For if the chair picks a different alternative $c$, it only tells us that the chair prefers $c$ to both $a$ and $b$, but it generally does not answer the question we are interested in. Taking $p = q = 1$, $m=4$, $r=2$, we see that such a completion may be impossible. Luckily, this problem can be worked around, at the price of increasing the number of voters. 

First build a temporary profile $P$ of $p+q$ voters with $a$ and $b$ ranked as just described. Complete it with $m-2$ alternatives ranked in arbitrary orders so that the resulting profile has complete strict rankings for each of the $p+q$ voter. Now we will make the other alternatives as bad as desired by adding voters to $P$ that appreciate $a$ and $b$ more than the other alternatives, thus building a profile $P'$ in which, whatever the weights, $c$ may not have a better score than $a$. Observe that if we add $\delta$ voters (with $\delta$ a natural number) that put $a$ at first rank and $b$ at second rank, and $\delta$ voters that put $b$ at first rank and $a$ at second rank, we do not change the difference of the scores of $a$ and $b$, and thus, observing the chair choosing $a$ or $b$ still answers the question. Choose arbitrarily a matching of the remaining $m-2$ alternatives to the ranks $3$ to $m$, say, $c \mapsto 3$, $d \mapsto 4$, and so on. The first $\delta$ supplementary voters, who rank $a$ first and $b$ second, rank the other alternatives as the just defined mapping suggests. The other $\delta$ supplementary voters, who rank $b$ first and $a$ second, use the opposite ranking for the remaining alternatives, say, $c \mapsto m$, $d \mapsto m-1$, and so on. We can now prove that, for $\delta = p+q$, and whatever the weight vector $\w \in \W$, no alternative, except possibly $b$, has a better score than $a$. (In fact, as the construction will exhibit, they are all worst than both $a$ and $b$, but the weaker fact is enough for our claim.)
...
\commentOC{TODO move this.} At this stage, we are content that a procedure exists to transform our questions to questions about winners of a PSR. Further studies would investigate, for example, what is lost in terms of elicitation efficiency when we are forced to restrain to realistic concrete questions, meaning, questions involving small profiles; or investigate the relationship between our understanding of scores as (cardinal) utility and (ordinal) scores as definition of a PSR.
...
%It also estimates how much information of that kind we would need if we assume the information would not be elicited experimentally using an active learning approach, but would be collected empirically as they randomly appear to us. (This estimate is however very rough as it is most relevant only when the observations follow the same random distribution as the one we chose.) 
...
%\footnote{ $\bar{\profile} \!=\! \arg \max_{\profile \in C(\pprofile)} s(y; \profile, \bar{\w}) \!-\! s(x; \profile, \bar{\w})$; and $\bar{\w} \!=\! \arg \max_{w \in W} s(y; \pprofile, w) \!-\! s(x; \pprofile, w)$ .} 
Define: 
\begin{align}
...
Therefore, whenever $\tau_{1} < \tau_{2}$ we ask a question to the chair, and compare componentwise $\bar{\w}$ and $\w^{\tau}$, the argmax of Eq. (\ref{eq:tau1}), to find the position $r$ that is most valuable to ask about (the one that maximizes $\abs{\bar{\w}_r - \w^{\tau}_r}$).  
Instead, if $\tau_{1} > \tau_{2}$ we ask a question to one of the agent, picking the alternative randomly as described above. \commentOC{We’d better check for each $j$ the difference between the difference of the ranks of $x$ and $y$ in the worst $v$ and in the best $v$ and pick a voter related to highest difference, pick $x$ or $y$ with $1/2$ chances, and pick an incomparable alternative equiprobably.}
...
Non-interleaved non-regret strategies (useful as benchmarks).	
	\begin{itemize}
