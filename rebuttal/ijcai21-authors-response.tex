...
\addtocounter{pagecount}{-4}
	...
	We thank the reviewers for their comments. We focus on factual errors and on the “questions for the authors”. \commentBN{Maybe: "We focus on factual errors and only on the questions reported in “questions for the authors”", or "We focus on factual errors when not better specified." and we write a Q before the questions.}
...
\commentBN{I removed "to the point that it cannot be considered a variant", it's arguable, maybe it's better to not focus on this.}
...
\commentBN{If we want to keep this we could summarize it as follows}
Yes, when we do not specify the name of the dataset we mean data generated following the procedure described in Sec 5. We designed several strategies in order to challenge one another and select the best strategy in the worst case (Impartial Culture). Once this was found to be Pessimistic, we tested it on real dataset.
